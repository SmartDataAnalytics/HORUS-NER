...
class Util(object):
    ...
    def process_ds_conll_format(self, dspath, dataset_name, token_index, ner_index, separator='\t'):
        ...
        try:
            ...
            has3NER = -1
            tot_sentences = 1
            sys.log.info(':: processing sentences...')
            ...
            with open(dspath) as f:
                for line in f:
                    ...
                    if line.strip() == '':
                        if len(tokens) != 0:
                            sys.log.debug(':: processing sentence %s' % str(tot_sentences))
                            sentences.append(
                                process_and_save_sentence(has3NER, s, dataset_name, tokens, tags_ner_y))
                            tokens = []
                            tags_ner_y = []
                            s = ''
                            has3NER = -1
            ...
            sys.log.info(':: %s sentences processed successfully' % str(len(sentences)))
            return sentences
        except Exception as error:
            sys.log.error('caught this error: ' + repr(error))
    ...
    def print_annotated_sentence(self, horus_matrix):
        ...
        sys.log.info(':: sentence annotated :: ')
        sys.log.info(':: KLASS 1 -->: ' + x1)
        sys.log.info(':: KLASS 2 -->: ' + x2)
        sys.log.info(':: KLASS 3 -->: ' + x3)
        sys.log.info(':: KLASS 4 -->: ' + x4)
        sys.log.info(':: KLASS 5 -->: ' + x5)
            ...
            with SQLiteHelper(self.config.database_db) as sqlcon:
                ...
                for index in range(len(horus_matrix)):
                    ...
                    if (horus_matrix[index][5] in definitions.POS_NOUN_TAGS) or horus_matrix[index][7] == 1:
                        if auxc%1000==0:
                            self.sys.log.debug(':: processing term %s - %s [%s]' % (str(auxc), str(len(horus_matrix)), term))

...
POS_TAGGER_NLTK = 1
POS_TAGGER_STANFORD = 2
POS_TAGGER_TWITTERNLP = 3

PUNCTUATION_AND_OTHERS = ('\'', '.', ',', '!', '?', ';', ':', ')')

# TweetNLP Framework
RUN_TAGGER_CMD = config.models_tweetnlp_java_param + \
                 " -jar " + config.models_tweetnlp_jar + \
                 " --model " + config.models_tweetnlp_model
...
NER_DATASETS = [
            ['ritter.train', config.dir_datasets + 'Ritter/', 'ner_short.txt.horusx'],

#TODO: need to re-design the whole framework to make it simpler/allow multithread
'''
below an attempt, stoped since don't have time how
need to change database updates and and etc..
I think a better solution would be simply saving the features (per token) to
an index after processing throuh a multithread architecture.
So, no SQL inserts/updates/etc in between...
i.e., get rid of SQLite and use something like elasticsearch, for instance
this is a big change in the project's architecture, but should make things much easier
in terms of adding new features, etc..
'''
...
def extract_features(index, self):
    """
    ...
    try:
        ...
        config.logger.info('extracting features for token/term %s - %s' % (str(index), term))
        ...
        with self.conn:
            ...
            if 1 == 1:
                ...
                for itxt in range(limit_txt):
                    try:
                        if rows[itxt][6] == 0 or rows[itxt][6] is None:  # not processed yet
                            merged_en, error_translation = self.__detect_and_translate(rows[itxt][2], rows[itxt][3],
                                                                                       rows[itxt][0], rows[itxt][4],
                                                                                       rows[itxt][5])
                            ...
                            cursor.execute(
                                SQL_TEXT_CLASS_UPD % (ret_bow[0], ret_bow[1], ret_bow[2], ret_bow[3], ret_bow[4],
                ...
                gpbtm = [np.sum(yytm[0][0], axis=0), np.sum(yytm[0][1], axis=0), np.sum(yytm[0][2], axis=0),
                         np.sum(yytm[0][3], axis=0)]
            ...
            self.config.logger.debug('TX statistics:'
                                     ...
                                     (str(self.horus_matrix[index][definitions.INDEX_TOT_TX_LOC]).zfill(2),
                                      ...
                                      "{:.2f}".format(
                                          self.horus_matrix[index][definitions.INDEX_TOT_TX_LOC_TM_CNN]).zfill(2),
                                      "{:.2f}".format(
                                          self.horus_matrix[index][definitions.INDEX_TOT_TX_ORG_TM_CNN]).zfill(2),
                                      "{:.2f}".format(
                                          self.horus_matrix[index][definitions.INDEX_TOT_TX_PER_TM_CNN]).zfill(2),
                                      "{:.2f}".format(
                                          self.horus_matrix[index][definitions.INDEX_DIST_TX_I_TM_CNN]).zfill(2),
        ...
        return self.horus_matrix[index]
...
class FeatureExtraction(object):
    ...
    def __export_data(self, data, path, format):
        try:
    ...
    def extract_features_from_conll_parallel(self, file, out_subfolder, label=None, token_index=0, ner_index=1):
        try:
            if file is None:
            ...
            if len(self.horus_matrix) > 0:
                ...
                for index in range(len(self.horus_matrix)):
                    auxi += 1
                    if (self.horus_matrix[index][definitions.INDEX_POS] in definitions.POS_NOUN_TAGS) or \
                            self.horus_matrix[index][definitions.INDEX_IS_COMPOUND] == 1:
                        job_args.append(int(index))
                self.config.logger.info('job args created: %s tokens/terms to be processed! starting multithreading'
                                        % str(len(job_args)))

                with poolcontext(processes=int(self.config.nr_thread)) as pool:
                    asyncres = pool.map(partial(extract_features, self=self), job_args)

                self.config.logger.info('feature extraction done!')
                final_out_file_name = self.util.path_leaf(file) + '_' + time.strftime("%Y%m%d%H%M%S") + ".horus"
                path = self.config.dir_output + out_subfolder + final_out_file_name
                self.__export_data(asyncres, path, 'tsv')
            else:
    ...
    try:
        if len(sys.argv) not in (1,2,3,4):
            ...
            if tot_args == 2:
                ...
                extractor.extract_features_from_conll_parallel('Ritter/ner_one_sentence.txt', exp_folder, label='ritter')
